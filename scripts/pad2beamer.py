...
def replace_special_chars(line):
    for orig, replacement in SPECIAL_TRANS_TABLE.iteritems():
        line = line.replace(orig, replacement)
    return line


def replace_special_chars_links(text):
    for orig, replacement in SPECIAL_TRANS_TABLE_LINKS.iteritems():
...
def handle_graphics(line):
    ...
    if m is not None:
        ...
        if not os.path.exists(image_path):
            ...
            # HACK: needed?
            url = urllib2.urlopen(urllib2.unquote(img_uri))
            with open(image_path, "wb") as w_file:
                w_file.write(url.read())
...
def runpad2beamer(argv):
    ...
    lines = correct_wikilines(raw_lines)
