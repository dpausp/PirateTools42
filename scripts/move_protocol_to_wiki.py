...
if __name__ == "__main__":
    #generate_wenterhaken_protocol_entries(date.today())
    #generate_stammtisch_protocol_entries(date.today())
    args = sys.argv[1:]
    for a in args:
        sep = a.split("/")
        if len(sep) > 1:
            ...
            session = a
            addon = None
        move_protocol_to_wiki(SESSION_TYPES[session], addon)
    #move_protocol_to_wiki(SESSION_TYPES["oberpfalz_vorstand"], "17-12-2012")
    #move_protocol_to_wiki(SESSION_TYPES["wenterhaken_offline"])
    #move_protocol_to_wiki(SESSION_TYPES["stammtisch_weiden"])
